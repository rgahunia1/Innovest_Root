...
from prediction_model_app.models import smbuser , doctor, sofi
      ...
      if request.POST['username'] and request.POST['email'] and  request.POST['name'] and request.POST['dob'] and request.POST['gender'] and request.POST['address']and request.POST['mobile']and request.POST['password']and request.POST['password1'] :
          ...
          if password == password1:
              if User.objects.filter(username = username).exists():
                ...
                patientnew = smbuser(user=user,name=name,dob=dob,gender=gender,address=address,mobile_no=mobile_no)
                patientnew.save()
...
def sign_in_patient(request):
    ...
    if request.method == 'POST':
          ...
          if user is not None :
              ...
              try:
                 if ( user.smbuser.is_business == True ) :
                     auth.login(request,user)
...
def savepdata(request,patientusername):
  ...
  if request.method == 'POST':
    ...
    smbuser.objects.filter(pk=puser.smbuser).update(name=name,dob=dobdate,gender=gender,address=address,mobile_no=mobile_no)
...
def sign_in_sofi(request):
  if request.method == 'GET':
    
    return render(request,'sofi/sign_in_page/index.html')
  ...
  if request.method == 'POST':
          ...
          if user is not None :
              ...
              try:
                ...
                if ( user.sofi.is_sofi == True ) :
                  auth.login(request,user)
                  
                  request.session['sofiusername'] = user.username

                  return redirect('sofi_ui')
               
              except :
                  messages.info(request,'invalid credentials')
                  return redirect('sign_in_sofi')
      ...
      return render(request,'sofi/signin_page/index.html')
